...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			int peterBalance = peterPaul.getOrDefault("Peter", 0);
			int paulBalance = peterPaul.getOrDefault("Paul", 0);

			if (peterBalance > 0 && paulBalance < 1000) {
				int transferAmount = Math.min(peterBalance / 2, paulBalance + peterBalance);
				peterPaul.put("Peter", peterBalance - transferAmount);
				peterPaul.put("Paul", paulBalance + transferAmount);
			}
		}
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			...
			if (peterBalance >= 5000 && paulBalance >= 10000) {
				int partnershipContribution = (peterBalance / 4) + (paulBalance / 4);
				peterPaul.put("Peter", peterBalance - (peterBalance / 4));
				peterPaul.put("Paul", paulBalance - (paulBalance / 4));
				peterPaul.put("Partnership", partnershipContribution);
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			newMap.put(firstLetter, lastLetter);
		}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		for (String word : words) {
			wordTimes.put(word, wordTimes.getOrDefault(word, 0) + 1);
		}

		Map<String, Boolean> countMap = new HashMap<>();
			for (String word : wordTimes.keySet()) {
				boolean moreThanOnce = wordTimes.get(word) >= 2;
				countMap.put(word, moreThanOnce);
			}
		return countMap;
