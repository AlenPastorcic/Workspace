    ...
    public void writeReplacedTextFile(File sourceFile, File destinationFile, String searchTerm, String replacementTerm) throws IOException {
        try (Scanner sourceScanner = new Scanner(sourceFile.getAbsoluteFile());
            ...
            while (sourceScanner.hasNextLine()) {
                ...
                destinationWriter.println(line.replaceAll(searchTerm, replacementTerm));
            }

        } catch (IOException e) {
            throw new IOException("Could not read or write file: " + e.getMessage(), e);
        }
    }

//        String fileName = userInput.nextLine();
//        // File newFile = new File(replacementTerm);
//        if (!newFile.exists()) {
