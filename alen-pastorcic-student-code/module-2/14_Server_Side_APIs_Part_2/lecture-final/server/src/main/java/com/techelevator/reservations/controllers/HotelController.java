...
public class RHotelController {
    ...
    public Reservation addReservation(@Valid @RequestBody Reservation reservation) {
        if (reservation.getGuests() == 3) {
            throw new ResponseStatusException(HttpStatus.EXPECTATION_FAILED, "Three's company");
        }
        else if (hotelDao.getHotelById(reservation.getHotelId()) == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Cannot make reservation bacause hotel does not exist");
        }
